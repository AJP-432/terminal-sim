...
class ShortestPathFinder:
    ...
    def initialize_map(self, game_state: Map):
        """Initializes the map
        ...
        self.game_map = [[Node() for x in range(ARENA_SIZE)] for y in range(ARENA_SIZE)]

    def navigate_multiple_endpoints(self, start_point, end_points, game_state: Map):
        ...
        """
        if game_state.contains_structure(start_point):
            return
        ...
        for i in range(ARENA_SIZE):
            for j in range(ARENA_SIZE):
                location = [i, j]
                if is_in_bounds((i, j)) and self.game_state.contains_structure(location):
    ...
    def _idealness_search(self, start, end_points):
        ...
        while not current.empty():
            ...
            for neighbor in self._get_neighbors(search_location):
                if not is_in_bounds(neighbor) or self.game_map[neighbor[0]][neighbor[1]].blocked:
                    continue
    ...
    def print_map(self):
        ...
        if not self.initialized:
            print("Attempted to print_map before pathfinder initialization. Use 'this_object.initialize_map(game_state)' to initialize the map first")
            return
